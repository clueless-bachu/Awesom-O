...
ThreatGen::ThreatGen(const ros::NodeHandle &node, int n, double threshold, bool useDetector) : nh(node)
{
	this->n = n;
...
void ThreatGen::generateTargets(bool TEST)
{
	...
	{
		std::vector<double> rand_point = {(double)(rand() % 6) - 2.5, (double)(rand() % 6) - 2.5};
		threats.push_back(rand_point);

		if (TEST == false)
		{
			std::string command =
				"rosrun gazebo_ros spawn_model -urdf -file $(rospack find awesomo)/urdf/ar_tag" + std::to_string(i + 1) +
				".urdf -x " + std::to_string(rand_point[0]) + " -y " + std::to_string(rand_point[1]) + " -z 0.25 -model ar_tag" + std::to_string(i + 1);
